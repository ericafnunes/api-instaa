...
const cadastrarUsuario = async (req, res) => {
    ...
    if (!username) {
        res.status(404).json("O campo username é obrigátorio");
    }

    if (!senha) {
        res.status(404).json("O campo senha é obrigatório");
    ...
    try {
        ...
        if (quantidadeUsuarios) {
            res.status(400).json("O username informado já existe");
        }
...
module.exports = {
    cadastrarUsuario, obterPerfil, atualizarPerfil
}
